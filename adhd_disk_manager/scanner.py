...
import os
from adhd_disk_manager.classifier import classify
from adhd_disk_manager.project_detector import detect_project_root
    ...
    for root in roots:
        for dirpath, dirnames, filenames in os.walk(root, topdown=True, followlinks=follow_symlinks):
            ...
            try:
                ...
                logger(f'skip folder stat: {p} {e}')
                continue

            folders_batch.append((str(p), str(p.parent), p.name, folder_stat.st_ctime, folder_stat.st_mtime, len(p.parts), len(filenames), len(dirnames), file_sizes_sum, '', 'protected' if is_project else 'none', '', scan_time))
