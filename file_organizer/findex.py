...
from drive_discovery import list_windows_drives, select_roots
from scanner import scan_paths
from search import text_search, by_ext, by_category
from reports import generate_reports
from planner import build_plan
from mover import execute_move
from rollback import list_batches, run_rollback
...
def main():
    ...
    sc=sub.add_parser('scan'); sc.add_argument('--all-drives', action='store_true'); sc.add_argument('--drive'); sc.add_argument('--root', action='append'); sc.add_argument('--config', default='config.yaml')
    ...
    if a.cmd=='drives': print('\n'.join(list_windows_drives()))
        ...
        b,m,f = execute_move(conn, reports_dir, rollback_dir, a.dry_run, a.yes_confirmation_string)
